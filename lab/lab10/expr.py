...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError('{0} is not bound to a value.'.format(self.string))
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        evaluated_call_name = self.operator.eval(env)
        evaluated_number = [operand.eval(env) for operand in self.operands]
        return evaluated_call_name.apply(evaluated_number)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        #Error: [copied_env[self.parameters] = value for value in arguments]
        for parameter, argument in zip(self.parameters, arguments):
            copied_env[parameter] = argument
