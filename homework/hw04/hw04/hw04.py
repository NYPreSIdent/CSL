...
def balanced(m):
    ...
    if is_weight(m):
        return True
    len1, weight1 = length(left(m)), total_weight(end(left(m)))
    len2, weight2 = length(right(m)), total_weight(end(right(m)))
    if len1 * weight1 == len2 * weight2:
...
def make_withdraw(balance, password):
    ...
    """
    "*** YOUR CODE HERE ***"
    typed_password, errors = [], 0
    def withdraw(amount, user_password):
        nonlocal balance, typed_password, errors
        if errors == 3:
            return "Your account is locked. Attempts: " + str(typed_password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(result) == str:
        return result
    def joint_withdraw(amount, user_password):
        if user_password == new_password:
            return withdraw(amount, old_password)
        return withdraw(amount, user_password)
...
def mul_interval(x, y):
    ...
    value in y."""
    p1 = lower_bound[0] * lower_bound[0]
    p2 = lower_bound[0] * upper_bound[1]
    p3 = upper_bound[1] * lower_bound[0]
    p4 = upper_bound[1] * upper_bound[1]
